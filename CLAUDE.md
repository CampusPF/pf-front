...
- **`services/admin/`** — admin panel CRUD (courses, categories, modules, lessons, users: role changes and soft delete) and `getAdminStats`, which counts full admin listings and degrades per metric. `adminErrorMessage` turns a 403 into the "teacher has no write permission yet" message.
