...
- **`app/(app)/`** — authenticated area (`/dashboard`): sidebar + topbar shell, wrapped in `RequireAuth`. `/dashboard/admin` (resumen, cursos, categorías) adds `RequireRole(["admin","teacher"])`; teachers only see their own courses, and categorías is admin-only. `RequireRole` is UX only — the backend's `RolesGuard` is the real check.
...
- **`services/progress/course-progress.service.ts`** — per-course enrollment + completed lessons (filtered from the `/me` endpoints), `setLessonCompleted` (PATCH if a record exists, POST otherwise) and `enrollInFreeCourse` (paid courses answer 402 — they go through checkout).
...
- **`services/admin/`** — admin panel CRUD (courses, categories, modules, lessons) and `getAdminStats`, which counts full admin listings and degrades per metric. `adminErrorMessage` turns a 403 into the "teacher has no write permission yet" message.
