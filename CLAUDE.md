...
`data/*.mock.ts` provides course/dashboard/lesson/progress fixtures. Courses/lessons/progress mocks are only used with `NEXT_PUBLIC_COURSES_SOURCE=mock`; dashboard streak/achievements/hours are always mock (no endpoint). `data/countries.ts` is generated by `scripts/gen-countries.mjs`.
