// Genera data/countries.ts: nombre en español (Intl) + código telefónico + bandera.
// ISO 3166-1 alpha-2 -> código de discado (lista ITU estándar).
const DIAL = {
  ...
  GA:"241",GB:"44",GD:"1473",GE:"995",GF:"594",GG:"44",GH:"233",GI:"350",GL:"299",GM:"220",GN:"224",GP:"590",GQ:"240",GR:"30",GT:"502",GU:"1671",GW:"245",GY:"592",
  HK:"852",HN:"504",HR:"385",HT:"509",HU:"36",
  ID:"62",IE:"353",IL:"972",IM:"44",IN:"91",IO:"246",IQ:"964",IR:"98",IS:"354",IT:"39",
  JE:"44",JM:"1876",JO:"962",JP:"81",
  KE:"254",KG:"996",KH:"855",KI:"686",KM:"269",KN:"1869",KP:"850",KR:"82",KW:"965",KY:"1345",KZ:"7",
  ...
  OM:"968",
  PA:"507",PE:"51",PF:"689",PG:"675",PH:"63",PK:"92",PL:"48",PM:"508",PR:"1787",PS:"970",PT:"351",PW:"680",PY:"595",
  QA:"974",
...
const list = Object.entries(DIAL)
  .map(([code, dialCode]) => {
    ...
    return { code, name, dialCode, flag: flagEmoji(code) };
  })
...
const body = list
  .map(
    (c) =>
      `  { code: "${c.code}", name: ${JSON.stringify(c.name)}, dialCode: "${c.dialCode}", flag: "${c.flag}" },`,
  )
  .join("\n");

const out = `/* Generado con scratchpad/gen-countries.mjs — nombres en español (Intl),
   códigos de discado de la lista ITU. Si hace falta actualizar algo,
   editá el script y regeneralo, no este archivo a mano. */

export interface Country {
  /** ISO 3166-1 alpha-2, ej. "AR". */
  code: string;
  /** Nombre en español, ej. "Argentina". */
  name: string;
  /** Código telefónico sin "+", ej. "54". */
  dialCode: string;
  /** Emoji de la bandera. */
  flag: string;
}
...
/** País por defecto del formulario (la audiencia del proyecto es Argentina). */
